...
*   **📥 收集草稿箱与历史待办池 (Capture Inbox & Backlog Tasks)**：支持“先存入草稿箱”功能快速卸载脑力。提供未处理草稿列表、侧边栏未读红点，以及可激活的**历史延迟待办池 (Backlog)**，保持今日任务高度聚焦。
*   **🌅 每日收盘仪式 (Daily Shutdown Ritual)**：每天 20:00 后自动触发（或手动点击开启）的四步导引式收盘仪式：回顾今日成果、清理/延迟残留任务、预热明天 focus，并提供温情的收尾金句，帮助大脑清空今日工作记忆。
*   **🎯 单一焦点模式 (Immersive Focus)**：提供全屏沉浸式行动卡片，内置可视化**倒计时番茄钟**。当所有待办清空时，进入温馨舒适的休息奖励状态，避免焦虑反弹。
*   **🧠 GROW 框架执行教练 (GROW Coaching)**：拒绝 ChatGPT 式的无尽闲聊，基于诊断卡片提供可能性和选项，找出阻碍你行动的“注意残留”与“项目死角”。
*   **⚙️ ADHD 情绪阻抗诊断与辅助 (ADHD Assist)**：可选开启时间感知辅助、跳过任务时的**情绪阻抗诊断芯片**（深度剖析逃避根源并给予 GROW 启发），以及柔和的断链语气保护。
*   **📈 行动动量与周环比分析 (Momentum Tracker)**：自动捕获你的行为启动潜伏期，提供**累计完成数、最大连击天数、启动时效的周环比趋势对比（如 “比上周多 2 个”）**。
...
| **触发机制** | 目标分解，手动调度 | **生活事件锚点 (anchor_time)** 触发，如：*“起床后”*、*“晚餐后”* |
| **心智负荷** | 高，每次启动需要消耗意志力决策 | 低，通过重复形成肌肉记忆与无意识行为 |
| **激励方式** | 完成的清空感，防拖延 | 连续行动天数（Streak 保护机制，跳过不惩罚，断链柔和语气） |
...
*   **极简主义代码**：我们提倡最直觉、清晰的代码实现，不为单次用途引入过重的抽象与第三方库。
