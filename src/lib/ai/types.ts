...
export const DecomposedTaskSchema = z.object({
  ...
  suggested_start_time: z.string().optional().nullable().transform(v => v ?? undefined).describe("对于时间敏感型任务，推荐的开始时间（如 '14:30'），非时间敏感任务可为空或省略")
});
...
export const DecomposeSchema = z.object({
  ...
  tasks: z.array(DecomposedTaskSchema).max(5).describe("今日优先执行的原子步骤列表，最多 5 条。仅包含今日焦点 Entity 的任务"),
  backlog_tasks: z.array(DecomposedTaskSchema).optional().describe("所有非今日焦点 Entity 的任务，以及今日待办任务过多时溢出的低优先级任务，统一存入待办池（Backlog）"),
